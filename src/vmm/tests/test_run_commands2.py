"""
Comprehensive unit tests for vmm/lib/run_commands.py
...
from vmm.lib.run_commands import (
    NotACyLoggerError,
...
# from vmm.lib.loggers import CyLogger, MockLogger, LogPriority as lp
...
class TestRunWithInit(unittest.TestCase):
    ...
    def test_defaults_use_mock_logger_when_no_logger_supplied(self):
        rw = RunWith()
        self.assertIs(rw.logger, CyLogger)
...
class TestCommunicate(unittest.TestCase):
    ...
    @patch("vmm.lib.run_commands.Popen")
...
class TestTimeoutMethod(unittest.TestCase):
    ...
    @patch("vmm.lib.run_commands.threading.Timer")
    @patch("vmm.lib.run_commands.Popen")
...
class TestRunWithSudoSubprocess(unittest.TestCase):
    ...
    @patch("vmm.lib.run_commands.os.set_blocking")
    @patch("vmm.lib.run_commands.select.select", return_value=([], [], []))
    @patch("vmm.lib.run_commands.subprocess.Popen")
    ...
    @patch("vmm.lib.run_commands.os.set_blocking")
    @patch("vmm.lib.run_commands.select.select")
    @patch("vmm.lib.run_commands.subprocess.Popen")
...
class TestRunMyThreadCommand(unittest.TestCase):
    ...
    @patch("vmm.lib.run_commands.RunThread")
